...
Master_Spine_Diameter_df = pd.DataFrame(columns = ['Spine Mean Diameter', 'ID', 'cell number', 'animal number','genotype'])
...
for i in range(len(folder_list)):
    ...
    Spine_Diameter_df = pd.DataFrame(reformat_row_list[1:])
    Spine_Diameter_df.columns = reformat_row_list[0]
    Spine_Diameter_df['cell number'] = folder_list[i][1:-11]
    ...
    Spine_Diameter_df['animal number'] = animal_number
    Spine_Diameter_df['genotype'] = genotype

    #delete irrelevant columns from dataframe
    Spine_Diameter_df = Spine_Diameter_df.drop(columns = ['Unit', 'Category', 'Depth', 'Level', 'Time', 'FilamentID',])

    #append to big dataframe
    Master_Spine_Diameter_df = Master_Spine_Diameter_df.append(Spine_Diameter_df)

#export to excel
Master_df.to_excel(r'/Users/lhaetzel/Desktop/Grad School/Rotations/Bateup Lab/Imaris_Export/Spine_Diameter_by_individual_spines.xlsx', index = False)
