...
Master_df = pd.DataFrame(columns = ['Spine Volume', 'ID', 'cell number', 'animal number','genotype'])
...
for i in range(len(folder_list)):
    ...
    Spine_Volume_df = pd.DataFrame(reformat_row_list[1:])
    Spine_Volume_df.columns = reformat_row_list[0]
    Spine_Volume_df['cell number'] = folder_list[i][1:-11]
    ...
    Spine_Volume_df['animal number'] = animal_number
    Spine_Volume_df['genotype'] = genotype

    #delete irrelevant columns from dataframe
    Spine_Volume_df = Spine_Volume_df.drop(columns = ['Unit', 'Category', 'Depth', 'Level', 'Time', 'FilamentID'])

    #append to big dataframe
    Master_df = Master_df.append(Spine_Volume_df)


#export to excel
Master_df.to_excel(r'/Users/lhaetzel/Desktop/Grad School/Rotations/Bateup Lab/Imaris_Export/Spine_Volume_by_individual_spines.xlsx', index = False)
