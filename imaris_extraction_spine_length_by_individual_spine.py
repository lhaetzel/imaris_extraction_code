...
Master_df = pd.DataFrame(columns = ['Spine Length', 'ID', 'cell number', 'animal number','genotype'])
...
for i in range(len(folder_list)):
    ...
    Spine_Length_df = pd.DataFrame(reformat_row_list[1:])
    Spine_Length_df.columns = reformat_row_list[0]
    Spine_Length_df['cell number'] = folder_list[i][1:-11]
    ...
    Spine_Length_df['animal number'] = animal_number
    Spine_Length_df['genotype'] = genotype

    #delete irrelevant columns from dataframe
    Spine_Length_df = Spine_Length_df.drop(columns = ['Unit', 'Category', 'Depth', 'Level', 'Time', 'FilamentID'])

    #append to big dataframe
    Master_df = Master_df.append(Spine_Length_df)


#export to excel
Master_df.to_excel(r'/Users/lhaetzel/Desktop/Grad School/Rotations/Bateup Lab/Imaris_Export/Spine_Length_by_individual_spines.xlsx', index = False)
