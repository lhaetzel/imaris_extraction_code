...
Master_df = pd.DataFrame(columns = ['Spine Area', 'ID', 'cell number', 'animal number','genotype'])
...
for i in range(len(folder_list)):
    ...
    Spine_Area_df = pd.DataFrame(reformat_row_list[1:])
    Spine_Area_df.columns = reformat_row_list[0]
    Spine_Area_df['cell number'] = folder_list[i][1:-11]
    ...
    Spine_Area_df['animal number'] = animal_number
    Spine_Area_df['genotype'] = genotype

    #delete irrelevant columns from dataframe
    Spine_Area_df = Spine_Area_df.drop(columns = ['Unit', 'Category', 'Depth', 'Level', 'Time', 'FilamentID'])

    #append to big dataframe
    Master_df = Master_df.append(Spine_Area_df)


#export to excel
Master_df.to_excel(r'/Users/lhaetzel/Desktop/Grad School/Rotations/Bateup Lab/Imaris_Export/Spine_Area_by_individual_spines.xlsx', index = False)
